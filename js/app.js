...
function setBoard(){
shuffleArray = shuffle(cardClass); //get the shuffled array
...
// function to add three starts on reset
function addStar(){
...
function reset(){
    ...
    countFlag = 0;
    oldClass = "";
    deckArea.innerHTML = ""; //remove older cards
    movesCount=1;
    timeCount = 0;
    firstClick = true;
    time.innerHTML = `0 secs`;// set the clock to zero
    movesindicator.innerHTML=`0`;// set the moves count to zero
    addStar();//set he stars to three
    setBoard();
}
...
// if cliked on cross close the modal
span.onclick = function(){
    modal.style.display = "none";
};
//if clicked outside modal close the modal
window.onclick = function(event){
    if(event.target == modal){
        modal.style.display = "none";
    }
};
setBoard(); // start the game by setting the board.
